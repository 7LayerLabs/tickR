...
export async function GET(
  ...
  try {
    ...
    const quoteRes = await fetch(
      `https://finnhub.io/api/v1/quote?symbol=${upperTicker}&token=${FINNHUB_API_KEY}`
    )
    ...
    if (!quoteData.c || quoteData.c === 0) {
      return NextResponse.json(
        { error: 'Stock not found', ticker: upperTicker },
        { status: 404 }
      )
    }
    ...
    const profileRes = await fetch(
      `https://finnhub.io/api/v1/stock/profile2?symbol=${upperTicker}&token=${FINNHUB_API_KEY}`
    )
    const profileData = await profileRes.json()

    // Check our local database for kid-friendly description
    const localData = STOCK_DATABASE[upperTicker]

    // Build the response
    const stockData = {
      ticker: upperTicker,
      companyName: localData?.name || profileData.name || upperTicker,
      price: quoteData.c,
      changePercent: quoteData.dp || 0,
      changeAmount: quoteData.d || 0,
      previousClose: quoteData.pc || 0,
      high: quoteData.h || 0,
      low: quoteData.l || 0,
      open: quoteData.o || 0,
      sector: localData?.sector || profileData.finnhubIndustry || 'Other',
      color: SECTOR_COLORS[localData?.sector || profileData.finnhubIndustry] || SECTOR_COLORS.Other,
      description: localData?.description || `${profileData.name || upperTicker} is a publicly traded company.`,
      ...
      // Indicate if we have full local data or partial
      hasFullData: !!localData,
    }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch stock data', ticker: upperTicker },
      { status: 500 }
    )
  }
}
